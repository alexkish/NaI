...
main(int argc, char **argv)
{
	...
	pRunManager->SetUserInitialization(new NaIDetectorConstruction);
	pRunManager->SetUserInitialization(new NaIPhysicsList);

	G4VisManager* pVisManager = new G4VisExecutive;
	pVisManager->Initialize();

	// create the primary generator action
	NaIPrimaryGeneratorAction *pPrimaryGeneratorAction = new NaIPrimaryGeneratorAction();

	// create an analysis manager object
	NaIAnalysisManager *pAnalysisManager = new NaIAnalysisManager(pPrimaryGeneratorAction);
	...
	pRunManager->SetUserAction(new NaIStackingAction(pAnalysisManager));
	pRunManager->SetUserAction(new NaIRunAction(pAnalysisManager));
	pRunManager->SetUserAction(new NaIEventAction(pAnalysisManager));
